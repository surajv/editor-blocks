( function( tinymce ) {
	...
	tinymce.PluginManager.add( 'wplink', function( editor ) {
		...
		editor.on( 'nodechange', function( event ) {
			editor.$( '.wp-has-caret' ).removeClass( 'wp-has-caret' );

			if ( event.element.nodeName === 'A' ) {
				editor.$( event.element ).addClass( 'wp-has-caret' );
			}
		} );

		// editor.on( 'keydown', function( event ) {
		// 	if ( event.keyCode === tinymce.util.VK.RIGHT ) {
		// 		var lastRng = editor.selection.getRng();
		// 		var lastNode = lastRng.startContainer;
		// 		var lastOffset = lastRng.startOffset;

		// 		if ( lastOffset + 1 === lastNode.data.length ) {
		...
		// 						var clonedRange = rng.cloneRange();

		// 						clonedRange.setStart( node, 0 );
		// 						clonedRange.setEnd( node, 0 );

		// 						editor.selection.setRng( clonedRange );
		// 					} );
		// 				}
		// 			} );
		// 		}
		...
		// 			if ( offset === 1 && parent.nodeName === 'A' ) {
		// 				if ( node === lastNode || parent === lastNode ) {
		// 					return;
		// 				}

		// 				var clonedRange = rng.cloneRange();

		// 				clonedRange.setStart( parent, 0 );
		// 				clonedRange.setEnd( parent, 0 );
